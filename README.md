...
2. `WSReq:Invoice` Составное поле описывающее заказ. Обязательное для заполнения
 * `WSReq:InvoiceNumber` Номер заказа, поле должно быть уникальным в базе данных магазина. Обязательное для заполнения
 * `WSReq:InvoiceStatus` Код статуса заказа. Указывается в виде 4-х значного числового кода. Обязательное для заполнения. Код “0000” значит, что магазин подтверждает проведение транзакции по данному номеру заказа. Любой другой код статуса будет восприниматься как отказ со стороны магазина.
 * `WSReq:InvoiceStatusDescription` Описание кода статуса заказа.
...
a. webshopResponse – Запрос в ПС от магазина, XML сообщение WebShopRequest.
b. merchantId – Уникальный номер магазина, выдается в момент заключения договора между магазином и ПС.
...
9. Когда отправляет? После перевода е-денег с кошелка на кошелек? 
